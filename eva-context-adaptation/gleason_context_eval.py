"""
GleasonArvaniti context evaluation: True apples-to-apples comparison.

This script produces context-enriched embeddings for Gleason in a way that ensures
a fair comparison with the baseline (single-patch OpenMidnight CLS token).

KEY INSIGHT: GleasonArvaniti dataset contains fixed-size patch images (750×750)
extracted from prostate TMAs.
Patches are classified into 4 classes: benign, Gleason pattern 3, 4, and 5.
The dataset is organized by split (train/validation vs. test), with patches grouped by
TMA core identifiers rather than explicit class folders.
GleasonArvaniti doesn't naturally provide metadata to trace position of patches within
...
1. Retrieve the patch metadata (centroid coordinates and TMA ID) from the CSV
2. Download the corresponding TMA from S3
...
Usage:
  python gleason_context_eval.py \
...
class GleasonContextDataset(Dataset):
    ...
    - "baseline": returns a single image patch corresponding to a Gleason patch.
    - "context": returns a grid of spatially adjacent patches extracted from the TMA core
    ...
    def __getitem__(self, idx: int):
        ...
        # Context mode
            ...
            # If TMA is bigger than region_size, center crop
            if orig_w >= region_size and orig_h >= region_size:
                left = (orig_w - region_size) // 2
                top = (orig_h - region_size) // 2
                tma_final = tma.crop((left, top, left + region_size, top + region_size))
    
                # Adjust centroid relative to crop
                cx -= left
                cy -= top

            # If TMA is smaller than region_size, resize it to region_size
            else:
                scale_w = region_size / orig_w
                scale_h = region_size / orig_h
                scale = max(scale_w, scale_h)  # ensure both dims ≥ region_size

                new_w = int(orig_w * scale)
                new_h = int(orig_h * scale)
    
                tma_resized = tma.resize((new_w, new_h), Image.BICUBIC)
                cx *= scale
                cy *= scale

                # Center crop after resizing
                left = (new_w - region_size) // 2
                top = (new_h - region_size) // 2
                tma_final = tma_resized.crop((left, top, left + region_size, top + region_size))
                cx -= left
                cy -= top

            # Compute grid index
            tile = self.model_input_size
            grid_col = int(cx // tile)
            grid_row = int(cy // tile)

            # clip to ensure inside grid
    ...
    def get_tma_id(self, idx: int) -> str:
        if isinstance(idx, torch.Tensor):
            idx = idx.item()

        patch_name = self.get_patch_name(idx)
        tma_id = patch_name.split("_patch")[0] 
        return tma_id
        ...
        for _, (patches, labels, indices) in enumerate(progress):
            ...
            for _, (emb, lbl, ds_idx) in enumerate(zip(embeddings, labels, indices)):
                ...
                records.append({
                    ...
                    "target": lbl,
                    "split": split,
                    "tma_id": dataloader.dataset.get_tma_id(ds_idx),
                })
...
def write_manifest(manifest_path: Path, records: List[Dict]) -> None:
    ...
    fieldnames = ["embeddings", "target", "split", "tma_id"]
...
def main():
    parser = argparse.ArgumentParser(description="GleasonArvaniti context evaluation - fair comparison")
    ...
    parser.add_argument("--eval-config",
                        default=str(_openmidnight_root / "eval_configs/gleason_embeddings_only.yaml"))
    parser.add_argument("--model-name", default="gleason_eval",
